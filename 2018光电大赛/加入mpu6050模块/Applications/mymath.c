...
REAL fast_atan2(REAL y, REAL x) //快速计算arctan的值
{
	REAL x_abs, y_abs, z;
...
//快速平方根算法
float my_sqrt(float number)  //平方根运算
{
	long i;
	float x, y;
	const float f = 1.5F;
...
#define ANGLE_UNIT (TWO_PI/10.0) //这里将36度作为一个单位

double mx_sin(double rad)    //角度的sin与运算
{   
	double sine;
	if (rad < 0)
...
float my_deadzone_p(float x,float zone)//这里的死区算法是将def固定为0，x小于zone范围，x固定为0
{
	float t;
	if(x>0)
	{
...
double To_180_degrees_db(double x)   //将度数范围设置为-180~180
{
...
float To_360_degrees(float x)         
{
	return (x>=360?0:(x<-1?359.0f:x));
}
