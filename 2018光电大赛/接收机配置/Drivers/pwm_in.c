...
/*TIM3和TIM4的输入捕获功能获取当前4个螺旋桨的PWM值，
对应每个螺旋桨的PWM存储其高电平的计数次数和低电平的
计数次数，最后通过设置的时钟换算
*/
void _TIM3_IRQHandler(void)	
{
	...
	Feed_Rc_Dog(1);//RC，400ms进入一次喂狗，对应操控遥控器的数据传输
	
	if(TIM3->SR & TIM_IT_CC1) 
	{
		TIM3->SR = ~TIM_IT_CC1;    //TIM_ClearITPendingBit(TIM3, TIM_IT_CC1);消除中断标记
		TIM3->SR = ~TIM_FLAG_CC1OF;//关闭重复捕获标记
		if(GPIOC->IDR & GPIO_Pin_6)//TIM3对应PC6，当获取第一个上升沿时记录当前定时器寄存器的值
		{
			temp_cnt1 = TIM_GetCapture1(TIM3);//获取定时器3输入捕获的值
		}
		else
		{
